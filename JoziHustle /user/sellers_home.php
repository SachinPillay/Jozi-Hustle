...
<head>
    <meta charset="UTF-8">
    <title>Seller Home - Jozi Hustle</title>
...
<div class="nav">
    <a href="sellers_home.php">Home</a>
    <a href="ad.php">Post Ad</a>
...
<div class="container">
    ...
    <div class="ad-grid">
        <?php
        $stmt = $conn->prepare("SELECT * FROM ads WHERE seller_id = ? ORDER BY id DESC");
        $stmt->execute([$seller_id]);
        $ads = $stmt->fetchAll(PDO::FETCH_ASSOC);

        foreach ($ads as $ad) {
            echo "<div class='ad-item " . ($ad['status'] == 'sold' ? "sold" : "") . "'>";
            ...
            if ($ad['status'] == 'sold') {
                echo "<p style='color: red; font-weight: bold;'>SOLD</p>";
            }

            // Buttons
            echo "<a href='ad_edit.php?id=" . htmlspecialchars($ad['id']) . "'>
                    <button class='action-btn edit-btn'>Manage</button>
                  </a>";

            echo "<form method='POST' action='update_status.php'>
                    <input type='hidden' name='ad_id' value='" . htmlspecialchars($ad['id']) . "'>
                    <button type='submit' name='status' value='" . ($ad['status'] == 'sold' ? "available" : "sold") . "' class='action-btn sold-btn'>
                        " . ($ad['status'] == 'sold' ? "Mark as Available" : "Mark as Sold") . "
                    </button>
                  </form>";

            echo "<a href='delete_ad.php?id=" . htmlspecialchars($ad['id']) . "' onclick=\"return confirm('Are you sure you want to delete this ad?');\">
